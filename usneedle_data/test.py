...
#print(args.root_path)
args.data = "custom"
#data_info = IPdata_test_parser[participant]
args.target = "Skill"
args.enc_in, args.dec_in, args.c_out = [13,13,1]
allFiles = []
for participant in IPdata_test_parser:
    for file in IPdata_test_parser[participant]['data']:
        allFiles.append(file)

print("------------------------------------------------------------------------------------------------------------------------------")
#print(allFiles)
exp = Exp(args)
exp.test2(origin_checkpoint_path + "/" + "IP", originPath + "/IP/", allFiles)

allFiles = []
for participant in OOPdata_test_parser:
    for file in OOPdata_test_parser[participant]['data']:
        allFiles.append(file)
...
#print(allFiles)
exp = Exp(args)
exp.test2(origin_checkpoint_path + "/" + "OOP", originPath + "/OOP/", allFiles)

"""

for participant in IPdata_train_parser:
    args.checkpoints = origin_checkpoint_path + "/" + "IP"
    args.root_path = originPath + "/IP/"
    #print(args.root_path)
    args.data = "custom"
    data_info = IPdata_train_parser[participant]
...
for participant in OOPdata_train_parser:
    args.root_path = originPath + "/OOP/"
    args.checkpoints = origin_checkpoint_path + "/" + "OOP"
    #print(args.root_path)
    args.data = "custom"
    data_info = OOPdata_train_parser[participant]
    args.target = data_info['T']
    args.enc_in, args.dec_in, args.c_out = data_info[args.features]
    for i in range(len(data_info['data'])):
        args.data_path = data_info['data'][i]
...
IPdata_train_parser.update(IPdata_val_parser)
OOPdata_train_parser.update(OOPdata_val_parser)
...
for participant in IPdata_test_parser:
    ...
    #args.data_path = data_info['data']
        ...
        exp.test2(origin_checkpoint_path + "/" + "IP", originPath + "/IP/", 'IP/' + args.data_path, data_info['data'])
        print("done testing")
"""
"""
participant in OOPdata_test_parser:
    args.checkpoints = origin_checkpoint_path + "/" + "OOP"
    args.root_path = originPath + "/OOP/"
    #print(args.root_path)
    args.data = "custom"
    data_info = OOPdata_test_parser[participant]
    args.target = data_info['T']
    args.enc_in, args.dec_in, args.c_out = data_info[args.features]
    for i in range(len(data_info['data'])):
        args.data_path = data_info['data'][i]
    ...
    #args.data_path = data_info['data']
        ...
        # test
        print('>>>>>>>start testing : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
        exp.test(setting,'OOP/'+ args.data_path)
        print("done testing")
